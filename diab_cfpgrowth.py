...
start = t.time()
patterns = cfp.generate_items_sets(ItemsData, mis.mis_values_ls(mis.support(ItemsData), 40, 0.1))
#print(mis.mis_values_ls(mis.support(ItemsData), 40, 0.1))
asso_roles =  cfp.generate_association_rules(patterns, 0)
end = t.time()
roles_file = open("cfpgrowth_asso_rules.txt", "w")
new_asso_roles = {}
for key in asso_roles.keys():
    if asso_roles[key][0] in [('1', ), ('0', )] or asso_roles[key] in [('1', ), ('0', )]:
